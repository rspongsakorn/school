...
### Dialog — ขั้นที่ 1 (เปิดทันที)

ตาราง **รูปแบบไฟล์** 3 คอลัมน์: **คอลัมน์ (key)** | **ความหมาย** | **ตัวอย่าง** (ตาม §3)

ปุ่ม **ดาวน์โหลดไฟล์ตัวอย่าง** — CSV มี header + 1 แถวตัวอย่างตาม spec

### Dialog — ขั้นที่ 2

- เลือกไฟล์ `.csv`  
- ถ้าเกิน 500 แถว → error ทั้งไฟล์ ไม่เข้าขั้น preview

### Dialog — ขั้นที่ 3 (หลัง parse)
...
- รหัสนักเรียนนี้มีในระบบแล้ว  
- รหัสนักเรียนซ้ำในไฟล์ (แถวนี้)  
- ไม่รู้จักคำนำหน้า/เพศ  
- รูปแบบวันเกิดไม่ถูกต้อง  
- วันเกิดเป็นวันในอนาคต  
- ข้อมูลไม่ครบ (ระบุฟิลด์)

**ตาราง preview** (สูงสุด 10 แถวแรกที่พร้อมนำเข้า):

| รหัส | ชื่อ-นามสกุล | เพศ (แสดง) | วันเกิด (แสดง พ.ศ.) |

ปุ่ม:

- **ยืนยันนำเข้า** — เปิดเมื่อ N > 0  
- **ยกเลิก** / ปิด

### Dialog — ขั้นที่ 4 (หลังยืนยัน)
